...
import { faultResolveSchema } from "../../../../../features/faults/lifecycle-schema";
...
export async function POST(
  ...
  try {
    ...
    const result = await resolveFault(
      reference,
      faultResolveSchema.parse(await request.json()),
      membership,
      ...
      return NextResponse.json(
        {
          ...
          message: "A resolution summary and valid version are required.",
    ...
    return NextResponse.json(
      {
        code,
        message: error instanceof Error ? error.message : "The fault could not be resolved.",
        requestId: requestContext.requestId,
      },
      { status },
    );
