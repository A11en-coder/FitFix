...
import { faultReopenSchema } from "../../../../../features/faults/lifecycle-schema";
...
export async function POST(
  ...
  try {
    ...
    const result = await reopenFault(
      reference,
      faultReopenSchema.parse(await request.json()),
      membership,
      ...
      return NextResponse.json(
        {
          ...
          message: "A reopen reason and valid version are required.",
    ...
    return NextResponse.json(
      {
        code,
        message: error instanceof Error ? error.message : "The fault could not be reopened.",
        requestId: requestContext.requestId,
      },
      { status },
    );
