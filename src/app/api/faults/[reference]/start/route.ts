...
import { faultStartSchema } from "../../../../../features/faults/lifecycle-schema";
...
export async function POST(
  ...
  try {
    ...
    const result = await startFault(
      reference,
      faultStartSchema.parse(await request.json()),
      membership,
    ...
    return NextResponse.json(
      {
        code,
        message: error instanceof Error ? error.message : "The fault could not be started.",
        requestId: requestContext.requestId,
      },
      { status },
    );
